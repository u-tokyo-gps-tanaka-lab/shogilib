...
def test_rank2l():
    ...
    (hands, onboards) = rank2l(
        79711464585613890499229372106470162629966919404028159691811738577084532
    )
    print(f"hands={hands}, onboards={onboards}")
    # assert False
...
def test_pos2l():
    pos = Position.from_fen(
        "+pg2n1+pnp/+p1g1+p2+p+p/p1P1b1b2/+p1slk3L/2+p1PGR2/3nKs1+p1/s1gPS+P3/P3n4/1lpr3LP[] w"
    )
    l = pos2l(pos)
    flipl = flipH_onboards(l[1])
    assert flipl < l[1]
    pos = Position.from_fen(
        "4k4/9/9/9/9/9/9/9/4K4[ppppppppppppppppppnnnnllllggggssssrrbb] w"
    )
    l = pos2l(pos)
    flipl = flipH_onboards(l[1])
    assert flipl == l[1]
